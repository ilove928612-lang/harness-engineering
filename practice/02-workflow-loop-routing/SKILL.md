...
本 skill 可同构安装到 5 端（Claude Code / Codex / Grok / Claw / Hermes），但依赖方并非每端都有：

| 依赖 | 所在端 | 缺失时行为 |
|------|--------|-----------|
| superpowers 链（brainstorming / writing-plans / tdd / requesting-code-review） | Claude、Hermes | 缺失端降级为「不触发闭环，直接回答」，不硬闯 |
| vloop（references/loop-protocol.md、configurator.md） | 仅 Hermes | 缺失端大任务退化为 superpowers 链；两端都没有则直接回答 |
| ponytail hook | 仅 Hermes（pre_llm_call） | 缺失端无影响，路由照常 |

> 判定顺序：两端依赖都在 → 按路由表走；仅 superpowers → 大任务也走 superpowers 链；两者都无 → 不触发闭环。L3 闸门在任何端都保留。
