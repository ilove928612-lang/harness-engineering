...
description: 把一批 Harness Engineering 调研候选（文章/论文/工具的 URL）走完「抓取→翻译→评审→收录→清理」流水线，整合进本仓库 works/ 与 references/articles.md，并保持 C1–C9 计数一致。当用户说"处理这批调研候选 / 收录这些链接 / 整理 translate / 把这几篇翻译收进来"时使用。这是仓库给自己用的策展 harness。
---
...
### ⑤ 校验 + 提交
- 跑 `bash scripts/check-consistency.sh`，C1–C9 必须全绿再提交。
